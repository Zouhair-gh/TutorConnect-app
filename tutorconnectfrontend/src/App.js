...
import SessionSummary from "./components/Sessions/VideoSessions/SessionSummary";
import SessionFeedback from "./components/Sessions/VideoSessions/SessionFeedback";
  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        ...
        const role = response.data.role?.trim().toUpperCase();
        const isParticipant =
          role === "PARTICIPANT" || role === "ROLE_PARTICIPANT";
  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        ...
        console.error("Token verification error:", error);
        localStorage.removeItem("authToken");
        setAuth({ loading: false, isValid: false });
        navigate("/login");
      }
    };
      ...
      >
        <Router>
          <Routes>
            ...
            {/* Video Session Routes - Accessible to both Tutors and Participants */}
            <Route
            ...
            {/* Session Summary Route - After session ends */}
            <Route
            ...
            <Route
              path="/tutor/sessions/:sessionId/video"
              element={
                <ProtectedTutorRoute>
                  <VideoConferenceWrapper isTutor={true} />
                </ProtectedTutorRoute>
              }
            />
